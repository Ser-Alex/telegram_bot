...
from states.state import HighSelect


@dp.message_handler(text='Максимальные значения', state='*')
@dp.message_handler(commands='high', state='*')
async def high_command(message: types.Message, state: dispatcher.FSMContext) -> None:
    """
    Функция для телеграм бота, начинает работу с любого состояния путём вызова команды /high
    или отправив в чат текст 'Максимальные значения'
    ...
    await state.set_state(HighSelect.category_selection.state)


@dp.callback_query_handler(dispatcher.filters.Text(endswith='_kb1'), state=HighSelect.category_selection.state)
async def selection_category(call: types.CallbackQuery, state: dispatcher.FSMContext) -> None:
    ...
    await state.set_state(HighSelect.quantity_selection.state)


@dp.message_handler(state=HighSelect.quantity_selection.state)
async def selection_quantity(message: types.Message, state: dispatcher.FSMContext) -> None:
    ...
    elements = sorted(elements, key=lambda x: x['max'], reverse=True)
    if len(elements) < int(message.text):
        ...
        history = f"{country} {city} high {user_data['category'].replace(' ', '')} {len(elements)}"
    ...
    history = f"{country} {city} high {user_data['category'].replace(' ', '')} {message.text}"
    set_history(message.from_id, history)
    await message.answer(text='Меню:', reply_markup=reply.keyboard1())
